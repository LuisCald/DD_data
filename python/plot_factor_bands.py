...
the DD_replication pipeline (`export_draws.jl`); it ships with this repo.
...
def main() -> None:
    ...
    if not os.path.exists(args.draws):
        raise SystemExit(
            ...
            "It ships with this repo — restore it from git, or regenerate via "
            "the DD_replication pipeline (export_draws.jl)."
        )

...
#
# FactorMap (julia/reconstruct.jl) learns the linear map factors ->
...
#
# Usage:
#   julia --project=. julia/posterior_bands.jl
...
# by the DD_replication pipeline (export_draws.jl). Only deps: CSV, DataFrames,
# Statistics, Plots.
# ─────────────────────────────────────────────────────────────

const HERE  = @__DIR__
const REPO  = abspath(joinpath(HERE, ".."))
const SYNTH = joinpath(REPO, "data")
...
function main()
    ...
    isfile(draws_csv) || error("Draws file not found: $draws_csv\n" *
        "It ships with this repo — restore it from git, or regenerate via the DD_replication pipeline (export_draws.jl).")
